...
__all__ = ['add_gradients_summaries',
           'create_clones',
           'deploy',
           'optimize_clones',
           'DeployedModel',
...
_deployment_params = {'num_clones': 1,
                      ...
                      'num_replicas': 1,
                      'num_ps_tasks': 0,
                      'gpu_start_index': 0,  # A hack for my dual gpu computer...
                      'worker_job_name': 'worker',
                      'ps_job_name': 'ps'}



def get_custom_getter(variable_dtype):
  ...
  def inner_custom_getter(getter, *args, **kwargs):
    ...
    if not (requested_dtype == tf.float32 and
                variable_dtype == tf.float16):
      # Only change the variable dtype if doing so does not decrease variable
      # precision.
      kwargs['dtype'] = variable_dtype
...
def create_clones(config, model_fn, variable_dtype=tf.float32, args=None, kwargs=None):
      ...
      with tf.name_scope(config.clone_scope(i)) as clone_scope:
        ...
        with tf.device(clone_device):
          with tf.variable_scope(tf.get_variable_scope(),
                                 custom_getter=get_custom_getter(variable_dtype),
...
def _gather_clone_loss(clone, num_clones, regularization_losses, loss_collection=tf.GraphKeys.LOSSES):
  ...
  with tf.device(clone.device):
    all_losses = []
    clone_losses = tf.get_collection(loss_collection, clone.scope)
    if clone_losses:
    ...
    if regularization_losses:
      ...
      if clone_loss is not None and regularization_loss.dtype !=clone_loss.dtype:
        regularization_loss = tf.cast(regularization_loss, clone_loss.dtype, name='regularization_loss_casted')
...
def _optimize_clone(optimizer, clone, num_clones, regularization_losses, gradient_scale=1.0,
  ...
  Returns:
    A tuple (clone_loss, clone_grads_and_vars).
      ...
      - clone_grads_and_vars: List of (gradient, variable) for the clone.
        Can be empty.
  """
  sum_loss = _gather_clone_loss(clone, num_clones, regularization_losses, loss_collection=loss_collection)
  clone_grad = None
  if sum_loss is not None:
    with tf.device(clone.device):
      sum_loss_scaled = sum_loss
      if gradient_scale != 1.0:
        sum_loss_scaled *= gradient_scale
      clone_grad = optimizer.compute_gradients(sum_loss_scaled, **kwargs)
      if gradient_scale != 1.0:
        clone_grad = [(grad * tf.cast(1. / gradient_scale, grad.dtype), var) for (grad, var) in clone_grad if grad is not None]

  return sum_loss, clone_grad


def optimize_clones(clones, optimizer, gradient_scale,
                    regularization_losses=None, loss_collection=tf.GraphKeys.LOSSES,
                    **kwargs):
  ...
  for clone in clones:
    with tf.name_scope(clone.scope):
      clone_loss, clone_grad = _optimize_clone(
        ...
        gradient_scale=gradient_scale, loss_collection=loss_collection, **kwargs)
      if clone_loss is not None:
  ...
  if len(clones_losses):
    total_loss = tf.add_n(clones_losses, name='total_loss')
  else:
    total_loss = tf.constant(0.0, name='total_loss')
...
def deploy(config,
  ...
  with tf.device(config.optimizer_device()):
    if optimizer:
      ...
      total_loss, clones_gradients = optimize_clones(clones, optimizer, gradient_scale, loss_collection=loss_collection)

      if clones_gradients:
        if summarize_gradients:
          # Add summaries to the gradients.
          summaries |= set(add_gradients_summaries(clones_gradients))
      ...
      for clone in clones:
        with tf.name_scope(clone.scope):
          clone_loss = _gather_clone_loss(clone, len(clones),
                                          regularization_losses, loss_collection=loss_collection)
          if clone_loss is not None:
            clones_losses.append(clone_loss)
          # Only use regularization_losses for the first clone
          regularization_losses = None
...
class DeploymentConfig(object):
  ...
  def __init__(self,
    ...
    self._gpu_start_index = gpu_start_index
  ...
  @property
  def gpu_start_index(self):
    return self._gpu_start_index
  ...
  def clone_device(self, clone_index):
    ...
    if self._clone_on_cpu:
      ...
      device += '/device:GPU:%d' % (clone_index + self._gpu_start_index)
    return device
  ...
  def clone_scope(self, clone_index):
    ...
    if self._num_clones > 1:
      scope = 'clone_%d' % (clone_index + self._gpu_start_index)
    return scope
  ...
  def optimizer_device(self):
    ...
    if self._num_ps_tasks > 0 or self._num_clones > 0:
      if self._clone_on_cpu:
        ...
        return self._worker_device + '/device:GPU:%d' %(self._gpu_start_index)
    else:
      return ''
