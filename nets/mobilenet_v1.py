    ...
    with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding='SAME'):
      ...
      for i, conv_def in enumerate(conv_defs):
        ...
        if isinstance(conv_def, Conv):
          ...
          if net.dtype.base_dtype == tf.float16:
            net = tf.cast(net, tf.bfloat16)
          net = slim.separable_conv2d(net, None, conv_def.kernel,
                                      ...
                                      normalizer_fn=slim.batch_norm,
                                      scope=end_point)
          if net.dtype.base_dtype == tf.bfloat16:
            net = tf.cast(net, tf.float16)
            raise NotImplementedError('Gives error No OpKernel was registered to support Op \'DepthwiseConv2dNative\''
                                      ' with these attrs. probably dtype mismatches.'
                                      ' separable_conv2d does not support fp16 yet.')
